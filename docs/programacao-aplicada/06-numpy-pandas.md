...
7. Pandas + NumPy: DataFrame a partir de arrays numpy
8. Estatísticas descritivas
9. Persistência com `st.session_state`
10. Streamlit + NumPy
...
## Pandas + NumPy — DataFrame a partir de Arrays
...
# Coluna calculada (relação resistência/peso)
...
- Exibir gráfico de barras com `st.bar_chart`
...
- Calcular tensão atuante com operações vetorizadas: $\sigma = \frac{F \times 1000}{A}$
- Exibir histograma das tensões (use `st.bar_chart` ou converta para DataFrame)
- Calcular e exibir $P(\sigma > 300)$ — fração de cenários que falham
...
## Exercício 6.8 — Mini-Projeto: Analisador de Ensaios Mecânicos
...
- Estatísticas por material (`groupby` do Pandas)
- Coluna calculada (ex: resistência específica = tensão / densidade)
